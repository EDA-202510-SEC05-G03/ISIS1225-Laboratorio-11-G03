...
from DataStructures.Priority_queue import edge as e
...
def new_graph(size, cmpfunction, directed, type, datastructure):
    """
    Crea un grafo vacio
    ...
    try:
        ...
        graph['vertices'] = map.new_map(numelements=size,
                                       maptype='PROBING',
                                       cmpfunction=cmpfunction)
        if (directed):
            graph['indegree'] = map.new_map(numelements=size,
                                           maptype='PROBING',
                                           cmpfunction=cmpfunction)
        return graph
...
def insert_vertex(graph, vertex):
    ...
    try:
        ...
        if not contains_vertex(graph, vertex):
            edges = lt.new_list('SINGLE_LINKED', e.compareedges) # incluye funcion comparacion arcos
...
def edges(graph):
    ...
    try:
        ...
        lstresp = lt.new_list('SINGLE_LINKED', e.compareedges)
        for lstedge in lt.iterator(lstmap):
            for edge in lt.iterator(lstedge):
                if (graph['directed']):
                    ...
                    invertedEdge = e.new_edge(vertexB, vertexA, e.weight(edge))
                    if (not lt.is_present(lstresp, edge)) and (not lt.is_present(lstresp, invertedEdge)):
    ...
    try:
        ...
        if edge is not None:  # caso arco YA existe
            ...
            if ((not graph['directed']) and (vertexa != vertexb)):
                inv_edge = get_edge(graph, vertexb, vertexa)
            ...
            lt.add_last(entrya['value'], edge)
            if not graph['directed']:
                if vertexa != vertexb:
                    inv_edge = e.new_edge(vertexb, vertexa, weight)
                    lt.add_last(entryb['value'], inv_edge)
            else:
...
def adjacents(graph, vertex):
    ...
    try:
        ...
        if element is not None:
            lst = element['value']
            for edge in lt.iterator(lst):
                lt.add_last(lstresp, e.other(edge, vertex))
        return lstresp
    except Exception as exp:
